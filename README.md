...
_**PBD**_($\lambda$). Let $N_k(j-1)$ denote the number of treatment $K$ assignments among first $j−1$ subjects, and $k^{(j-1)} = \min_{1\leq k \leq K}\left\{int\left(\frac{N_k(j-1)}{w_k}\right)\right\}$ denote the number of minimal balanced sets among the first $j−1$ assignments.
